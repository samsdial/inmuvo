...
if(modalname!='error'&&modalname!='scontent'&&modalname!='lpr'){var errortext='';var errortype=0;};if(modalname=='scontent'){var scontenttype=errortype;var scontenttext=errortext;var errortext='';var errortype=0;};if(errortype!=2){if(status=='open'){if($.pfdialogstatus=='true'){$("#pf-membersystem-dialog").dialog("close");}
$('#pf-membersystem-dialog').pfLoadingOverlay({action:'show'});var minwidthofdialog=380;if(modalname=='terms'){minwidthofdialog=650;}
if(!$.pf_mobile_check()){minwidthofdialog=320;};$.ajax({type:'POST',dataType:'html',url:theme_scriptspf.ajaxurl,data:{'action':'pfget_usersystem','formtype':modalname,'security':theme_scriptspf.pfget_usersystem,'errortype':errortype,'scontenttype':scontenttype,'scontenttext':scontenttext,'lang':theme_scriptspf.pfcurlang,'redirectpage':redirectpage,'pid':pid},success:function(data){$("#pf-membersystem-dialog").html(data);$('#pf-login-trigger-button-inner').click(function(){$.pfOpenLogin('open','login')});$('#pf-register-trigger-button-inner').click(function(){$.pfOpenLogin('open','register')});$('#pf-lp-trigger-button-inner').click(function(){$.pfOpenLogin('open','lp')});$('.pftermshortc').on('click',function(event){setTimeout(function(){$.pfOpenLogin('open','terms');},0);});$('#pf-ajax-terms-button').on('click',function(event){$.pfOpenLogin('open','register');return false;});if(modalname=='error'){$('#pf-ajax-cl-details').html(errortext);};if(modalname=='lpr'){$('.pf-additional-detailsforlpr').html(errortext);};$('#pf-ajax-loginfacebook').click(function(){$('#pf-membersystem-dialog').pfLoadingOverlay({action:'show'});});$('#pf-ajax-logintwitter').click(function(){$('#pf-membersystem-dialog').pfLoadingOverlay({action:'show'});});$('#pf-ajax-logingoogle').click(function(){$('#pf-membersystem-dialog').pfLoadingOverlay({action:'show'});});$('#pfsocialconnectbutton').click(function(){var form=$('#pf-ajax-login-form');var pfsearchformerrors=form.find(".pfsearchformerrors");if($.isEmptyObject($.pfAjaxUserSystemVars)){$.pfAjaxUserSystemVars={};$.pfAjaxUserSystemVars.username_err='Please write username';$.pfAjaxUserSystemVars.username_err2='Please enter at least 3 characters for Username.';$.pfAjaxUserSystemVars.password_err='Please write password';}
