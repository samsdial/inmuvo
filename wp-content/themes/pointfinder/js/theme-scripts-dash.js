(function($) {
	...
	***************************************************************************************************************/
				  ...
				  marker:{
				    ...
				    events:{
				    	dragend: function(marker){
				    		...
				    		if ($.pfstreetview_status == 'active') {
					    		$('#pfitempagestreetviewMap').data('pfcoordinateslat',marker.getPosition().lat());
					    		$('#pfitempagestreetviewMap').data('pfcoordinateslng',marker.getPosition().lng());
					    	}
			...
			setTimeout(function(){
				...
				google.maps.event.addListener(autocomplete, "place_changed", function() {
				    ...
				    if ($.pfstreetview_status == 'active') {
				        ...
				        $.pfpanoramamap.setPosition(latLng);
						$.pfstviewgetnow(100);
					}
				});
			},1000);
						...
						geocoder.geocode({"latLng": latLng}, function(results, status) {
						    if (status == google.maps.GeocoderStatus.OK) {
						      if (results[0]) {
		    					...
		    					$('#pfitempagestreetviewMap').data('pfcoordinateslat',latLng.lat());
				    			$('#pfitempagestreetviewMap').data('pfcoordinateslng',latLng.lng());
			...
			$.ajax({
				...
				if (obj.features == null || obj.features == '' || obj.features == 'undefined') {
					...
					$.each($('[name="pffeature[]"]'),function(index, el) {
						...
						var status_of_attr = parent_element.attr('checked');
						if (parent_val == "yes") {
							if (status_of_attr == 'checked') {
	...
	/* IMAGE AND FILE UPLOAD STARTED */
		...
		/* Delete Cover Image */
		$(".pf-delete-coverimg-old").live("click", function(){
	...
	$(function(){
		...
		if ($.pfstreetview_status == 'active') {
			$.pfcormarker = $("#pfupload_map").gmap3({get: {id: "Perpignan"}});


			$.pfstviewgetnow = function(delayt){
				setTimeout(function(){
		          $.pfpanoramamap = $("#pfitempagestreetviewMap").gmap3({get: {name: "streetviewpanorama"}});
		          ...
		          $.pfmapHeading = $.pfpanoramamap.getPov().heading;
		          $.pfmapPitch = $.pfpanoramamap.getPov().pitch;
		          $.pfmapZoom = $.pfpanoramamap.getPov().zoom;

		          setTimeout(function(){
		            $.pfpanoramamap.addListener('pov_changed', function() {
		              $("#webbupointfinder_item_streetview-heading").val($.pfpanoramamap.getPov().heading);
		              $("#webbupointfinder_item_streetview-pitch").val($.pfpanoramamap.getPov().pitch)
		              $("#webbupointfinder_item_streetview-zoom").val($.pfpanoramamap.getPov().zoom)
		            });
			...
			google.maps.event.addListener( $.pfcormarker, 'dragend', function ( event )
	        {
	          ...
	          $.pfpanoramamap.setPosition(event.latLng);
	          $.pfstviewgetnow(100);
	        ...
	        $.pfstmapregenerate = function(){
	          ...
	          if ($('.pfupload_map').data( 'pf-lng') == 'undefied') {
	          	var defaultlocl = new google.maps.LatLng(40.71275, -74.00597);
	          }else{
	          	var defaultlocl = new google.maps.LatLng($('.pfupload_map').data( 'pf-lat'), $('.pfupload_map').data( 'pf-lng'));
	          }
