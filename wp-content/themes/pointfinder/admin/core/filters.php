...
*/
    ...
    add_action( 'login_form_rp', 'pointfinder_redirect_to_custom_password_reset' );
    add_action( 'login_form_resetpass', 'pointfinder_redirect_to_custom_password_reset' );
    add_action( 'login_form_lostpassword', 'pointfinder_redirect_to_custom_lostpassword' );

    function pointfinder_redirect_logged_in_user( $redirect_to = null ) {
    ...
    function pointfinder_redirect_to_custom_lostpassword() {
        if ( 'GET' == $_SERVER['REQUEST_METHOD'] ) {
            if ( is_user_logged_in() ) {
                pointfinder_redirect_logged_in_user();
                exit;
            }
     ...
     * if there are errors.
     */
    function pointfinder_redirect_to_custom_password_reset() {
        if ( 'GET' == $_SERVER['REQUEST_METHOD'] ) {
    ...
    add_action('wp_footer','pointfinder_lp_system_handler',400);
    function pointfinder_lp_system_handler(){
        ...
        switch ($scontenttype) {
            case 'lp':
                $scontenttext = esc_html__('lp','pointfindert2d');
                break;
            case 'lpex':
                $scontenttext = esc_html__('lpex','pointfindert2d');
                break;
            case 'lpin':
                $scontenttext = esc_html__('lpin','pointfindert2d');
                break;
            case 'lpr':
...
function pointfinder_review_fix_pagination(){
    $setup3_pointposttype_pt1 = PFSAIssetControl('setup3_pointposttype_pt1','','pfitemfinder');
    if (is_singular($setup3_pointposttype_pt1)) {
        remove_filter('template_redirect', 'redirect_canonical');
        remove_action( 'template_redirect', 'redirect_canonical' );
    }
}
add_action('template_redirect','pointfinder_review_fix_pagination',0);

function pointfinder_tag_fix_pagination(){
    if (is_tag()) {
        remove_filter('template_redirect', 'redirect_canonical');
        remove_action( 'template_redirect', 'redirect_canonical' );
    }
}
add_action('template_redirect','pointfinder_tag_fix_pagination',0);


function pointfinder_tags_page_fix( $query ) {
        ...
        if( $query->is_tag() && $query->is_main_query() ){
            ...
            $ppp = intval($ppp);
            $query->set( 'post_type', array( $setup3_pointposttype_pt1 ) );
...
add_action( 'pre_get_posts', 'pointfinder_tags_page_fix',10,1);
