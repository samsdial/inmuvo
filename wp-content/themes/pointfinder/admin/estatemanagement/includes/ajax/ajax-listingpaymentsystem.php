...
function pf_ajax_listingpaymentsystem(){
  ...
  /* Start: Defaults from Options Panel */
    ...
    $twocho_status = PFPGIssetControl('2cho_status','','0');
    $payf_status = PFPGIssetControl('payf_status','','0');
    */
    global $pfpgcontrol_options;
    $pags_status = $pfpgcontrol_options['pags_status'];
    $payu_status = $pfpgcontrol_options['payu_status'];
    $ideal_status = $pfpgcontrol_options['ideal_status'];
    $robo_status = $pfpgcontrol_options['robo_status'];
    $iyzico_status = $pfpgcontrol_options['iyzico_status'];
    $twocho_status = $pfpgcontrol_options['2cho_status'];
    $payf_status = $pfpgcontrol_options['payf_status'];
    ...
    if ($total_pr != 0) {
      ...
      if ($twocho_status == 1) {
        ...
        <div class="pf-lpacks-upload-option">
          ...
          <label for="pfm-payment-t2co">'.esc_html__('2Checkout Payment System','pointfindert2d').'</label>
          <div class="pfm-active">
          <p>'.esc_html__("Pay via 2Checkout; you can pay with your 2Checkout account.",'pointfindert2d').'</p>
          </div>
        </div>';
      }
      ...
      if ($payf_status == 1) {
        ...
        <div class="pf-lpacks-upload-option">
          ...
          <label for="pfm-payment-payf">'.esc_html__('PayFast Payment System','pointfindert2d').'</label>
          <div class="pfm-active">
          <p>'.esc_html__("Pay via PayFast; you can pay with your PayFast account.",'pointfindert2d').'</p>
          </div>
        </div>';
      }
