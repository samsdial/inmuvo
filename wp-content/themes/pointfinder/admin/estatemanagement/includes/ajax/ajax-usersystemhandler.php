...
if(!function_exists('pf_ajax_usersystemhandler')){
  function pf_ajax_usersystemhandler(){
    ...
    if (wp_get_referer()) {
  	...
  	switch($formtype){
  		case 'login':
        if (is_array($vars)) {
          $redirectpage = pointfinder_redirectionselect();
        ...
        if($progressok == 1){
          ...
          if ( $user_exist || $user_email_exist ) {
              ...
              if ( $message_reply) {
                  ...
                  if ($auto_ll == 1) {
                    if ($regsysx == 1) {
                      $message = esc_html__("Success! You will be auto login in 3sec.","pointfindert2d");
                    }else{
                    ...
                    if ($regsysx == 1) {
                      $message = esc_html__("Success! Please click here for login.","pointfindert2d");
                    }else{
                  ...
                  $message = esc_html__("User created. Mail Configuration not correct. Please check Mail Config Panel > Email Settings under PF Settings","pointfindert2d");
        ...
        if (!empty($vars['password']) && !empty($vars['password2'])) {
          ...
          if ( ! $user || is_wp_error( $user ) ) {
              ...
              if ( $user && $user->get_error_code() === 'expired_key' ) {
                  echo json_encode( array( 'reset'=>true, 'mes'=>sprintf(esc_html__('The password reset link you used is not valid anymore. Please %sclick here%s for create new password key.','pointfindert2d'),'<a class="lplink" href="'.$redirect_url.'">','</a>')));
              } else {
                  echo json_encode( array( 'reset'=>true, 'mes'=>sprintf(esc_html__('The password reset link you used is not valid anymore. Please %sclick here%s for create new password key.','pointfindert2d'),'<a class="lplink" href="'.$redirect_url.'">','</a>')));
              }
              die();
          ...
          if ( isset( $vars['password'] ) ) {
              if ( $vars['password'] != $vars['password2'] ) {
                  echo json_encode( array( 'reset'=>false, 'mes'=>esc_html__( 'The two passwords you entered don\'t match','pointfindert2d' )));
                  die();
              }
              ...
              echo json_encode( array( 'reset'=>true, 'mes'=>esc_html__('Password reset successful, now you can login with your new password. Please click here for open login window.','pointfindert2d' )));
          } else {
              echo json_encode( array( 'reset'=>false, 'mes'=>esc_html__( 'The password reset link you used is not valid anymore.','pointfindert2d' )));
          }
        }else{
          echo json_encode( array( 'reset'=>false, 'mes'=>esc_html__( 'Password is empty.','pointfindert2d' )));
        }
      break;
