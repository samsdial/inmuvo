				...
				if (count($terms) > 0) {
					...
					foreach ($terms as $term) {
						...
						if ($term->parent == 0) {
							...
							if ($controlvalue != 1) {
								$output_check = pointfinder_features_tax_output_check($term_parent,$id,$controlvalue);
							}else{
								$output_check = "ok";
							}
								...
								if ($term_child_count > 0) {
				                    ...
				                    foreach ($term_childs as $featured_terms_single_sub) {
						        		...
						        		$output_features .= '<span class="goption">';
					                    	$output_features .= '<label class="options">';
					                            $output_features .= '<input class="pffeature'.$term->term_id.'" type="checkbox" id="pffeature'.$term_chl->term_id.'" name="pffeature[]" data-pid="'.$term->term_id.'" data-parent="no" data-child="yes" data-childof="'.$term->term_id.'" value="'.$term_chl->term_id.'"'.$checked_text.' disabled>';
					                            $output_features .= '<span class="checkbox"></span>';
					                        $output_features .= '</label>';
