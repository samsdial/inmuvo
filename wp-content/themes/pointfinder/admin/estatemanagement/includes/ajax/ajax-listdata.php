...
function pf_ajax_list_items(){
	...
	header('Content-Type: text/html; charset=UTF-8;');
		...
		/* Get default Grid settings from admin */
			...
			if ($pf_from == 'halfmap') {
				if ($pfaction == 'search') {
					$setup22_dlcfcx = PFSAIssetControl('setup42_dlcfcx','','2');
				} else {
				...
				*/
					...
					if(is_array($pfgetdata)){
						...
						$pfformvars = array();
							...
							/* Added with v1.8.8 */
							/* Define Coordinates */
							if (isset($pfformvars['ne'])) {
								if(isset($pfformvars['ne'])){$ne = esc_attr($pfformvars['ne']);}else{$ne = 360;}
								if(isset($pfformvars['ne2'])){$ne2 = esc_attr($pfformvars['ne2']);}else{$ne2 = 360;}
								if(isset($pfformvars['sw'])){$sw = esc_attr($pfformvars['sw']);}else{$sw = -360;}
								if(isset($pfformvars['sw2'])){$sw2 = esc_attr($pfformvars['sw2']);}else{$sw2 = -360;}
							} else{
								if(isset($_POST['ne']) && $_POST['ne']!=''){$ne = esc_attr($_POST['ne']);}else{$ne = 360;}
								if(isset($_POST['ne2']) && $_POST['ne2']!=''){$ne2 = esc_attr($_POST['ne2']);}else{$ne2 = 360;}
								if(isset($_POST['sw']) && $_POST['sw']!=''){$sw = esc_attr($_POST['sw']);}else{$sw = -360;}
						...
						/* Changed values by user (Order / Sort / paging number) */
							if($pfg_orderby != ''){
								...
								if($pfgetdata['orderby'] != ''){
									...
									$args['orderby'][$pfgetdata['orderby']]= $order_user_dt;
								}else{
									if ($setup22_searchresults_defaultsortbytype == 'rand') {
										$args['orderby']['rand']= '';
									}else{
							...
							if ($pfgetdata['pfrandomize'] == 'yes') {$args['orderby']['rand']='';}
													   	...
													   	$wpflistdata .= '<label for="pfsearch-filter" class="lbl-ui select pfsortby">';
														   	$wpflistdata .= '<select class="pfsearch-filter" name="pfsearch-filter" id="pfsearch-filter">';
																	...
																	if(!empty($pfsearchvars)){
																		if(!isset($pfg_orderby)){
																			$wpflistdata .= PFIF_SortFields_sg($pfsearchvars);
																		}else{
																			$wpflistdata .= PFIF_SortFields_sg($pfsearchvars,$pfg_orderby);
																		}
																		...
																		if(!isset($pfg_orderby)){
																			$wpflistdata .= PFIF_SortFields_sg($pfgetdatax_r);
																		}else{
																			$wpflistdata .= PFIF_SortFields_sg($pfgetdatax_r,$pfg_orderby);
																		}
																	}else{
							                    ...
							                    /* Start: Conditions */
							                        ...
							                        if ($setup3_pt14_check == 1) {
			                        					...
			                        					if (isset($post_condition[0]->term_id)) {
					                        				...
					                        				$contidion_colors = pf_get_condition_color($post_condition[0]->term_id);
