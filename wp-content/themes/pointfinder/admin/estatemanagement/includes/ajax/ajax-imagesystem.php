		...
		if (get_current_user_id() == $post_author) {
			...
			if ($id != '') {
				...
				if (PFControlEmptyArr($images_of_thispost) || !empty($imageID_of_featured)) {
					...
					$output_images .= '<img src="'.pointfinder_aq_resize($image_src[0],90,90,true).'">';
				...
				if (isset($images_of_thispost[0])) {
					if (!empty($images_of_thispost[0])) {
						foreach ($images_of_thispost as $image_number) {
							...
							$output_images .= '<li>';
							$output_images .= '<div class="pf-itemimage-container">';
							$output_images .= '<img src="'.pointfinder_aq_resize($image_number['url'],90,90,true).'">';
							$output_images .= '<div class="pf-itemimage-delete"><a class="pf-delete-coverimg'.$olduptext.'" data-pfimgno="'.$image_number['id'].'" data-pfpid="'.$id.'" data-pffeatured="no">'.esc_html__( 'Remove', 'pointfindert2d' ).'</a></div>';
							$output_images .= '</div>';
							$output_images .= '</li>';
						}
						$output .= '<section class="pfuploadform-mainsec">';
							$output .= '<label for="file" class="lbl-text">'.esc_html__('Uploaded Header Image','pointfindert2d').':</label>';
							$output .= '<ul class="pfimages-ul">'.$output_images.'</ul>';
						$output .= '</section>';
