...
**/
	...
	if (!empty($st9_currency_status)) {
		...
		function pointfinder_currency_schedule(){
			...
			if (!empty($st9_currency_from)) {
				...
				if (!empty($st9_currency_to)) {
					...
					$currency_output_arr = pointfinder_custom_currencyConverter_clayer($st9_currency_from,$currency_arr);
				}
		...
		function pointfinder_custom_currencyConverter_clayer($currency_from,$currency_to){
			...
			$access_key = '2d5650ea03d31ac6c7c74003261127a3';
		    ...
		    $query_url = $base_url . "?access_key=".$access_key;
