				...
				switch ($params['formtype']) {
											...
											if ($setup4_mem_terms == 1) {
												...
												$this->FieldOutput .= '
													<span class="goption upt">
					                                    ...
					                                    <label for="check1">'.sprintf(esc_html__( 'I have read the %s terms and conditions %s and accept them.', 'pointfindert2d' ),'<a href="'.$terms_permalink.'" class="pftermshortc"><strong>','</strong></a>').'</label>
					                               </span>
												';
					...
					case 'edititem':
						...
						**/
							...
							if($params['formtype'] == 'upload'){
								...
								$buttontext = esc_html__( "Submit Listing", "pointfindert2d");
							...
							/*** DEFAULTS FOR FIRST COLUMN ***/
								...
								$setup4_coviup = PFSAIssetControl('setup4_coviup','','0');
								$setup4_coviup_req = PFSAIssetControl('setup4_coviup_req','','0');
								$st4_sp_medst = PFSAIssetControl('st4_sp_medst','','0');
							...
							if ($main_submit_permission) {
								...
								**/
										...
										if ($st4_sp_medst == 1) {
											$this->ScriptOutput .= "$.pfstreetview_status = 'active';";
										}else{
											$this->ScriptOutput .= "$.pfstreetview_status = 'deactive';";
										}
										...
										**/
											...
											if($st4_sp_med == 1){
												...
												if($st4_sp_medst == 1){
									                ...
									                $pointfinder_google_map_zoom = PFSAIssetControl('setup5_mapsettings_zoom','','1');
													...
													$this->FieldOutput .= '<section class="pfsubmit-inner-sub">';
													$this->FieldOutput .= '<label for="pfitempagestreetviewMap" class="lbl-text">'.esc_html__('STREET VIEW:','pointfindert2d' ).'</label>';
										...
										**/
											...
											if ($setup4_coviup == 1) {
												...
												$this->FieldOutput .= '<div class="pfsubmit-title pfsubmit-inner-sub-cimage">'.esc_html__('HEADER IMAGE UPLOAD','pointfindert2d' ).'</div>';
												...
												**/
													...
													"use strict";
														$(function(){
															...
															$this->FieldOutput .= '$.pfitemdetail_listcoverimages_old('.$params['post_id'].');';
															}
													$this->FieldOutput .= '
														});
													...
													$this->FieldOutput .= '<label for="file" class="lbl-text"></label><small style="margin-bottom:4px;display:block;">'.esc_html__('Recommended size width: 2000px and height: 485 px. (Better for large screens.)','pointfindert2d').'</small>';
													$this->FieldOutput .= '<div class="pfuploadcoverimg-container"><a id="pfuploadcoverimg_remove" style="font-size: 12px;line-height: 14px;"><i class="pfadmicon-glyph-64" style="font-size: 14px;"></i> '.esc_html__('Remove Uploaded Header Image','pointfindert2d').'</a></div>';
						                            ...
						                            <label for="file" class="lbl-ui file-input">
							                            <div id="pfcoverimageuploadcontainer">
													        <a id="pfcoverimageuploadfilepicker" href="javascript:;"><i class="pfadmicon-glyph-512"></i> '.esc_html__('Choose Header Image','pointfindert2d').'</a>
													    </div>
						                            </label>
						                            </div>
													';
													...
													if($setup4_submitpage_featuredverror_status == 1 && $params['formtype'] != 'edititem'){
														...
														if($this->VSOMessages != ''){
															$this->VSOMessages .= ',pfuploadcovimagesrc:"'.$setup4_submitpage_featuredverror.'"';
														}else{
															$this->VSOMessages = 'pfuploadcovimagesrc:"'.$setup4_submitpage_featuredverror.'"';
														}
													...
													$this->ScriptOutput .= "$.pfuploadcoverimagelimit = ".$images_newlimit.";";
													...
													if ($params['formtype'] == 'edititem') {
														...
														if ($.pfuploadcoverimagelimit <= ".$images_newlimit."){
													...
													/*Image upload featured image AJAX */
														var CoverfileInput = new mOxie.FileInput({
												            ...
												            container: 'pfcoverimageuploadcontainer',
												            accept: [{title: 'Image files', extensions: 'jpg,gif,png'}],
												            multiple: false
												        });

												        $.pfuploadedcoverfilecount = 0;
														$.pfuploadmaxcoverimage = 1;
												        $.pfuploadcoverimages = function(id){
												        	var numberi = id;
												        	numberi = numberi + 1;
											        		$('.pfitemcoverimgcontainer').pfLoadingOverlay({action:'show',message: '".esc_html__('Uploading image: ','pointfindert2d')."'+numberi});

															if ($.pfuploadcoverimagelimit > 0 && $.pfuploadedcoverfilecount < $.pfuploadmaxcoverimage) {
															    ...
															    formData.append('pfuploadcoverimg', CoverfileInput.files[id]);
															    ...
															    var clearcoversinterval = function(){
															    	...
															    	if ($.pfuploadedcoverfilecount == $.pfuploadmaxcoverimage) {
																    	if ($.pfuploadcoverimagelimit > 0) {
																    		$('.pfuploadcoverimgupl-container').css('display','inline-block');
																    	}else{
																    		$('.pfuploadcoverimgupl-container').css('display','none');
																    ...
																    $.pfuploadcoverimages($.pfuploadedcoverfilecount);
															    }
																...
																$('.pfuploadcoverimg-container').css('display','inline-block');
																$('.pfitemcoverimgcontainer').pfLoadingOverlay({action:'hide'});
																$.pfuploadmaxcoverimage = $.pfuploadedcoverfilecount = 0;
															};
												        };

												        CoverfileInput.onchange = function(e) {
												        	if (CoverfileInput.files && CoverfileInput.files.length) {
													       		$.pfuploadmaxcoverimage = CoverfileInput.files.length;
													       		$.pfuploadcoverimages(0);
												       		}
												        };


												        CoverfileInput.onready = function(e) {
													...
													/* Remove Featured Image Ajax */
														$('#pfuploadcoverimg_remove').live('click touchstart',function(){

															$('.pfitemcoverimgcontainer').pfLoadingOverlay({action:'show',message: '".esc_html__('Removing image...','pointfindert2d')."'});

														    var formData = new mOxie.FormData();
												            formData.append('action','pfget_imageupload');
														    formData.append('security','".$nonceimgup."');
														    formData.append('oldup',1);
														    formData.append('exid', $('#pfuploadcovimagesrc').val());
					...
					case 'profile':
						...
						**/
								...
								$usnfield = PFSAIssetControl('usnfield','','1');
																	        	...
																	        	$this->FieldOutput .= '<select name="paymenttype">';
																	        		if (PFSAIssetControl('setup20_paypalsettings_paypal_status','','1') == 1) {
																		       		...
																		       		if (($pointfinder_order_recurring != 1 && $pointfinder_order_frecurring != 1) && PFPGIssetControl('2cho_status','',0) == 1) {
																		       			$this->FieldOutput .= '<option value="t2co">'.esc_html__('2CHECKOUT','pointfindert2d').'</option>';
																		       		}

																		       		if (($pointfinder_order_recurring != 1 && $pointfinder_order_frecurring != 1) && PFPGIssetControl('payf_status','',0) == 1) {
																		       			$this->FieldOutput .= '<option value="payf">'.esc_html__('PAYFAST','pointfindert2d').'</option>';
																		       		}
