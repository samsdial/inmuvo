							...
							**/	
								if (isset($_GET['payf'])) {
									...
									if (esc_attr( $_GET['payf'] ) == 'success') {
										$sccval .= esc_html__('Thanks for your payment. PayFast payment process completed. ','pointfindert2d');

										if ($setup4_membersettings_paymentsystem == 2) {
											$sccval .= esc_html__('Thanks for your payment. PayFast payment process completed. Please wait for auto page refresh.','pointfindert2d');
														
											$sccval .= '<script>setTimeout(function(){window.location.reload(1);}, 3000);</script>';
										}
									
									}else{
										$errorval .= esc_html__('Unfortunately PayFast payment process not completed.','pointfindert2d');
									}
									...
									if (!empty($robo_check2)) {
										...
										if ($result) {
											...
											if (esc_attr( $_GET['credit_card_processed'] ) == 'Y') {
												$sccval .= esc_html__('Thanks for your payment. 2Checkout payment process completed. ','pointfindert2d');
											}else{
												$errorval .= esc_html__('Unfortunately 2Checkout payment process not completed.','pointfindert2d');
											}
											...
											if (!empty($order_id)) {
												...
												pointfinder_directpayment_success_process(
													array(
												        ...
												        'otype' => $otype,
												        'user_id' => $user_id,
														'paymentsystem_name' => esc_html__("2Checkout","pointfindert2d"),
												...
												if ($setup4_membersettings_paymentsystem == 2) {
													$sccval .= esc_html__('Thanks for your payment. 2Checkout payment process completed. Please wait for auto page refresh.','pointfindert2d');
