...
**/
	...
	/* Changed with v1.8.8.8 and PFIF_SortFields_ld deprecated*/
		...
		function PFIF_DetailText_ld($id,$setup22_searchresults_hide_lt,$post_listing_typeval=''){
			...
			if($pfstart == true){
				...
				if($setup22_searchresults_hide_lt == 0){
					...
					<ul class="pointfinderpflisttermsgr"><li>'.GetPFTermInfo($id,'pointfinderltypes').'</li></ul></div>';
				}
			}
			unset($PF_CF_Val);
...
**/
	...
	if (!function_exists('PFreCaptchaWidgetLP')) {
		function PFreCaptchaWidgetLP(){
			$wpf_rndnum = rand(10,1000);
			$publickey = PFRECIssetControl('setupreCaptcha_general_pubkey','','');
			$lang = PFRECIssetControl('setupreCaptcha_general_lang','','en');
			return '<script type="text/javascript">var PFFonloadCallback = function() {jQuery.widgetIdlp = grecaptcha.render("g-recaptcha-lp", {"sitekey" : "'.$publickey.'"});};</script><div id="g-recaptcha-lp" class="g-recaptcha-field" data-rekey="widgetIdlp"></div><script type="text/javascript" src="https://www.google.com/recaptcha/api.js?hl='.$lang.'&onload=PFFonloadCallback&render=explicit" async defer></script>';
		}
	}
...
if (!function_exists('pointfinder_features_tax_output_check')) {
	function pointfinder_features_tax_output_check($term_parent,$listting_id,$controlvalue){
		$output_check = '';
			...
			if($itemvars['featured_image'] != ''){
				$output_data .= "<div class='wpfimage clearfix'><div class='wpfimage-wrapper clearfix'>";
					...
					if($setup10_infowindow_hover_image == 1 && $single_point == 0){
						...
						$output_data .= '<div class="pfImageOverlayH"></div><div class="pfButtons '.$buton_q_text.' pfStyleVAni"><span class="'.$pfbuttonstyletext.' clearfix"><a class="pficon-imageclick" data-pf-link="'.$itemvars['featured_image_big'].'" data-pf-type="image" style="cursor:pointer"><i class="pfadmicon-glyph-684"></i></a></span>';
						
						if($setup10_infowindow_hover_video != 1 && !empty($itemvars['featured_video'])){			
						$output_data .= '<span class="'.$pfbuttonstyletext.'"><a class="pficon-videoclick" data-pf-link="'.$itemvars['featured_video'] .'" data-pf-type="iframe" style="cursor:pointer"><i class="pfadmicon-glyph-573"></i></a></span>';
						}
				            ...
				            if($params['fieldsubtype'] == 4){
				            	...
				            	$output .= '<script>
											...
											$("[name=\''.$params['fieldname'].'\']").rules( "add", {
											  number: true,
											  messages: {
											    number: "'.esc_html__( "Please add numeric value and only dot (.) allowed for decimals.", "pointfindert2d" ).'"
											  }
											});})(jQuery);</script>';
...
/** 
*Start: ajax-listingtypes.php Functions
**/
	if(!function_exists('Pointfinder_GET_LType_FUP')){
		function Pointfinder_GET_LType_FUP($params = array()){
		    $defaults = array( 
		        'listname' => '',
		        'listtype' => '',
		        'listtitle' => '',
...
/** 
*End: ajax-listingtypes.php Functions
**/
if (!function_exists('pointfinder_clear_invoice_amount')) {
	function pointfinder_clear_invoice_amount($amount){
		...
		if ($setup20_decimals_new > 0 && !is_numeric($amount)) {
...
if (!function_exists('pointfinder_featured_image_getresized_ex')) {
	function pointfinder_featured_image_getresized_ex($listing_meta,$listingtype_single,$template_directory_uri,$general_crop2,$general_retinasupport,$setupsizelimitconf_general_gridsize1_width,$setupsizelimitconf_general_gridsize1_height){
		$noimg_url = $template_directory_uri.'/images/noimg.png';
		$featured_image = '';
...
if (!function_exists('pointfinder_twoco_item_suspend')) {
	function pointfinder_twoco_item_suspend($order_id, $item_post_id, $user_id, $setup33_emaillimits_listingexpired,$setup4_membersettings_paymentsystem,$reason){
...
if (!function_exists('pointfinder_Utf8_ansi')) {
	function pointfinder_Utf8_ansi($valor='') {
...
if (!function_exists('pointfinder_sanitize_output')) {
	function pointfinder_sanitize_output($buffer) {
...
if (!function_exists('pointfinder_redirectionselect')) {
	function pointfinder_redirectionselect(){

		$homeurllink = site_url($path = '/');
        $pfmenu_perout = PFPermalinkCheck();

		$redfreg = PFSAIssetControl('redfreg','','1');
		$redfregc = PFSAIssetControl('redfregc','','');
...
/* Query cleanup addedwith v1.8.8.8 */
add_filter( 'pointfinder_cleanup_query_for_grid', 'pointfinder_query_cleanup_filter',10,1);
if (!function_exists('pointfinder_query_cleanup_filter')) {
	function pointfinder_query_cleanup_filter($args){
