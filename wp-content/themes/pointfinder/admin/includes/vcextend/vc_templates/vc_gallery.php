...
$pretty_rel_random = 'pfgallery'.rand();	
...
foreach ( $images as $attach_id ) {
	...
	if($thumbnail === false) {
		if($general_retinasupport == 1){
			$thumbnail = pointfinder_aq_resize($post_thumbnail[0],$featured_image_width/2,$featured_image_height/2,true);
			if($thumbnail === false) {
    ...
    if ( $onclick == 'link_image' ) {
        $link_start = '<a class="wbp_vc_gallery_pfwrapper clearfix '.$pretty_rel_random.'" href="'.$p_img_large.'">';
...
(function($) {
	"use strict";
	$(function() {
		$(".'.$pretty_rel_random.'").magnificPopup({
		  type: "image",
		  gallery:{
		    enabled:true
		  }
